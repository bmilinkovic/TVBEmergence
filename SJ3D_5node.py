...
data_directory = os.path.join(results_directory, 'SJ3D_5node/data/')
figure_directory = os.path.join(results_directory, 'SJ3D_5node/figures/')
...
idx = np.r_[random.sample(range(0, 75), 5)]         # <-- get a row and columnar index for specific regions or nodes


changed_weights = np.array([[3, 3, 3, 0, 0],         # <-- configure weights and tract lengths for the subset of nodes
                           [3, 3, 3, 0, 0],
                           [3, 3, 3, 0, 0],
                           [0, 0, 0, 3, 3],
                           [0, 1, 0, 3, 3]])
...
subset = connectivity.Connectivity(weights=changed_weights,
...
def run_sim(global_coupling, noise):
    ...
    for i in range(len(changed_weights)):
        data_cleaned[i] = zscore(np.sum(data[:, 0, i, :], axis=1))
    return (global_coupling, noise, data_cleaned, time)


global_coupling_log = 10**np.r_[-2:-0.02:25j] # <-- set the coupling range -2:-0.5:25j
noise_log = 10**np.r_[-3:-0.0:25j] # <-- set the noise range -3:-0.002:25j
...
for i in range(len(data)):
        sio.savemat(data_directory + 'SJ3D_5node_gc-{0:02f}_noise-{1:02f}.mat'.format(float(data[i][0]), float(data[i][1])), {'data': data[i][2]}) # <-- save the data as a matlab file
...
for i in range(len(data)):
    ...
    plt.savefig(f"{figure_directory}/SJ3D_5node_gc-{data[i][0]}_noise-{data[i][1]}.png", dpi=300, bbox_inches='tight') # <-- save the figure
    plt.savefig(f"{figure_directory}/SJ3D_5node_gc-{data[i][0]}_noise-{data[i][1]}.eps", format='eps', dpi=300, bbox_inches='tight') # <-- save the figure
    plt.clf()
